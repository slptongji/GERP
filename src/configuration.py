...
class OptimizerConfig(object):
    ...
    def get_optimizer(self):
        ...
        if opt_name == "sgd":
            return tf.compat.v1.train.GradientDescentOptimizer(self.lr)
        elif opt_name == "sgdmomentum":
            return tf.compat.v1.train.MomentumOptimizer(self.lr, self.param["momentum"])
        elif opt_name == "adagrad":
            return tf.compat.v1.train.AdagradOptimizer(self.lr)
        elif opt_name == "adadelta":
            return tf.compat.v1.train.AdadeltaOptimizer(self.lr, self.param["decay_rate"])
        elif opt_name == "adam":
            return tf.compat.v1.train.AdamOptimizer(self.lr)
        else:
