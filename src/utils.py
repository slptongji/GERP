...
def uniform_initializer_variable(width, shape, name=""):
    var = tf.Variable(tf.random.uniform(shape, -width, width), name=name)
    return var


def truncated_normal_initializer_variable(width, shape, name=""):
    var = tf.Variable(tf.compat.v1.random.truncated_normal(shape=shape, stddev=1.0 / tf.sqrt(float(width))), name=name, dtype=tf.float32)
    return var
